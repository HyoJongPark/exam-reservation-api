...
def register(
    user: UserCreateRequest, db: Session = Depends(get_db_from_request)
) -> UserCreateResponse:
...
@router.post("/login")
def login(
    email: str, password: str, db: Session = Depends(get_db_from_request)
) -> Token:
    return user_service.login(db, email, password)
